...
// `ManagedAgentRecord` and its serde-only supporting types are shared with
// `buzz-cli` (`agents import`) via `buzz-agent-record` — see that crate for
// the canonical definitions. Re-exported here so every existing
...
/// Desktop-side view onto a [`ManagedAgentRecord`] — an inherent `impl` isn't
/// possible since the type now lives in `buzz-agent-record` (orphan rule), so
/// this extension trait keeps the `record.to_definition_view()` call syntax
/// at every existing site instead of a wider free-function rewrite.
pub trait ManagedAgentRecordExt {
    fn to_definition_view(&self) -> Option<AgentDefinition>;
}

impl ManagedAgentRecordExt for ManagedAgentRecord {
...
// `RespondTo`, `validate_respond_to_allowlist`, and the `DEFAULT_*` constants
// moved to `buzz-agent-record` (re-exported above) alongside `ManagedAgentRecord`.
