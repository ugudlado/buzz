...
desktop-release-build target="aarch64-apple-darwin":
    ...
    if [[ "$TARGET" != *windows* ]]; then
        touch "desktop/src-tauri/binaries/buzz-backend-host-$TARGET"
...
test-unit:
    ...
    if command -v cargo-nextest &>/dev/null; then
        ...
        cargo nextest run -p buzz-backend-host
    else
...
dev *ARGS: bootstrap _ensure-sidecar-stubs _ensure-migrations
    ...
    cargo build -p buzz-acp -p buzz-agent -p buzz-backend-host -p buzz-backend-kubernetes -p buzz-dev-mcp -p buzz-cli -p git-credential-nostr -p buzz-relay
...
desktop-standalone *ARGS: _ensure-sidecar-stubs
    ...
    set -euo pipefail
    export PATH="{{justfile_directory()}}/bin:$PATH"
    cargo build -p buzz-acp -p buzz-agent -p buzz-backend-host -p buzz-backend-kubernetes -p buzz-dev-mcp -p buzz-cli -p git-credential-nostr
    TARGET=$(rustc -vV | sed -n 's|host: ||p')
    TARGET_DIR=$(cargo metadata --format-version 1 --no-deps | node -p "JSON.parse(require('fs').readFileSync(0, 'utf8')).target_directory")
    for bin in buzz-acp buzz-agent buzz-backend-host buzz-backend-kubernetes buzz-dev-mcp git-credential-nostr buzz; do
...
staging *ARGS: bootstrap _ensure-sidecar-stubs
    ...
    cargo build --release -p buzz-acp -p buzz-agent -p buzz-backend-host -p buzz-backend-kubernetes -p buzz-dev-mcp -p buzz-cli -p git-credential-nostr
    FEATURES=()
    if [[ -n "{{mesh}}" ]]; then
        FEATURES=(--features mesh-llm)
    fi
    # Replace 0-byte sidecar stubs with real binaries so tauri dev picks them up.
    # buzz: the CLI sidecar. Backend providers: provider discovery scans the
    ...
    if [[ "$TARGET" != *windows* ]]; then
        STAGING_SIDECARS+=(buzz-backend-host buzz-backend-kubernetes)
    fi
...
production *ARGS: bootstrap _ensure-sidecar-stubs
    ...
    if [[ "$TARGET" != *windows* ]]; then
        PRODUCTION_SIDECARS+=(buzz-backend-host buzz-backend-kubernetes)
    fi
